...
# PUT → パラメータなし
# レスポンスの形式
# 200 → 利用再開後のアカウント情報(id/name/email/role/isActive)を返す
# 403 → 管理者以外のロールでログイン中
# 404 → 対象ユーザーが存在しない、または対象が管理者ロールのいずれか
# 422 → 対象が既に有効
# 500 → DB更新処理自体が失敗(コミットエラー)
...
def test_activate_user_returns_200(
    client: TestClient,
    db_session: Session,
    role: UserRoleType,
    name: str,
    ...
    create_user_and_login(db_session, client, role=UserRoleType.ADMIN)
    target_user = create_user(db_session, name=name, email=email, role=role, is_active=False)

    response = client.put(f"/api/v1/admin/users/{target_user.id}/activate")
...
def test_activate_user_with_non_admin_login_returns_403(
    client: TestClient,
    db_session: Session,
    login_role: UserRoleType,
) -> None:
    create_user_and_login(db_session, client, role=login_role)
    target_user = create_user(
        db_session, name="社員花子", email="employee@example.com", is_active=False
    )
...
def test_activate_user_returns_404(
    client: TestClient,
    db_session: Session,
    target_is_admin: bool,
) -> None:
    ...
    response = client.put(f"/api/v1/admin/users/{target_user_id}/activate")
...
# 既に有効なユーザーを指定した場合は422
def test_activate_user_with_already_active_target_returns_422(
    client: TestClient, db_session: Session
) -> None:
    create_user_and_login(db_session, client, role=UserRoleType.ADMIN)
    target_user = create_user(
    ...
    assert response.status_code == 422
    assert response.json()["detail"] == "既に有効なユーザーです"
...
# DB更新処理自体が失敗した場合は500・rollbackが呼ばれる
def test_activate_user_returns_500_when_commit_fails(
    db_session: Session,
    client_with_commit_error: TestClient,
    rollback_tracker: RollbackTracker,
) -> None:
    ...
    response = client_with_commit_error.put(f"/api/v1/admin/users/{target_user.id}/activate")
